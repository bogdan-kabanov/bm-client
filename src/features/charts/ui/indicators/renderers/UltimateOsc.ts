...
import { calculateUltimateOscillator } from '@src/shared/lib/indicators';

export const UltimateOscRenderer: IndicatorRenderer = {
  name: 'UltimateOsc',
  minCandles: 20,
  
  render(context: IndicatorRenderContext) {
    const { ctx, xScale, candles, chartArea } = context;
    const uo = calculateUltimateOscillator(candles);
    
    // Ultimate Oscillator от 0 до 100, отображаем в нижних 20% графика
    ...
    ctx.strokeStyle = 'rgba(96, 125, 139, 0.3)';
    ctx.lineWidth = 1;
    ctx.setLineDash([2, 2]);
