...
import { calculateWilliamsR } from '@src/shared/lib/indicators';

export const WilliamsRRenderer: IndicatorRenderer = {
  name: 'WilliamsR',
  minCandles: 20,
  ...
  render(context: IndicatorRenderContext) {
    ...
    // Williams %R от -100 до 0, отображаем в нижних 20% графика
    ...
    wr.forEach((value, index) => {
      if (!isNaN(value)) {
        // Преобразуем -100..0 в 0..1
        const normalizedValue = (value + 100) / 100; // 0-1
    ...
    ctx.lineWidth = 1;
    ctx.setLineDash([2, 2]);
    const level20Y = oscBottom - (0.8 * oscHeight); // -20 соответствует 0.8
    const level80Y = oscBottom - (0.2 * oscHeight); // -80 соответствует 0.2
    ctx.beginPath();
